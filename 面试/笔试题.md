...
const myInstanceof2 = (instance, constructor) => {
    return constructor?.prototype?.isPrototypeOf(instance) || false
}
```
...
## 实现一个函数sum，接受多个参数，sum(a)(b)...(n)()，返回a到n的和。
```js
const sum = (a) => {
...
## 实现一个函数sum，接受多个参数，sum(a)(b)...(n).count()，调用count()返回a到n的和。
```js
const sum = (a)=> {
    let args = [a];
...
class ConsoleLogOnBuildWebpackPlugin {
    ...
    apply(compiler) {
        ...
        compiler.hooks.emit.tap(pluginName, (compilation) => {
            console.log('所有打包生成的文件内容已经在内存中按照相应的数据结构处理完毕,下一步会将文件内容输出到output目录!')
        });
