...
单链表(LinkedList)是一种链式存取的数据结构，链表中的数据是以结点来表示的，每个结点的构成：元素 + 指针(指示后继元素存储位置)
...
双向链表(DoublyLinkedList)在单链表的基础上，对于每一个结点设计一个前驱结点，前驱结点与前一个结点相互连接，构成一个链表。它的每个数据结点中都有两个指针。

### js 实现

不同点：

- Node 中添加了一个 this.prev 属性, 该属性用于指向上一个节点
- 属性中添加了一个 this.end 属性, 该属性指向末尾的节点
...
循环链表(CircularLinkedList)和非循环链表其实创建的过程以及思路几乎完全一样，唯一不同的是，非循环链表的尾结点指向 null，而循环链表的尾指针指向的是链表的开头。
